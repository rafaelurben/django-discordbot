...
class Help(commands.Cog):
    ...
    @app_commands.describe(
        search="Befehl oder Kategorie", subsearch="Unterbefehl"
    )
    async def help(
        self,
        ...
        description = [
            "Syntax: `/help [Kategorie/Befehl/Befehl Unterbefehl]`\n\n"
        ]
        fields = []

        def addCog(
            cog, include_subcommands=False, hide_cogs=config.HELP_HIDDEN_COGS
        ):
            if not cog.qualified_name.lower() in hide_cogs:
                commands_list = ""
                for cmd in list(cog.walk_commands()) + list(
                    cog.walk_app_commands()
                ):
        ...
        def addGroup(grp):
            ...
            fields.append(
                (
                    "Unterbefehle von " + grp.qualified_name,
                    commands_list + "\u200b",
                )
            )
            ...
            if isinstance(cmd, (commands.Group, app_commands.Group)):
                if subsearch:
                    subcmds = dict(
                        ...
                        for alias in cmd.aliases + [cmd.name.lower()]
                    )
