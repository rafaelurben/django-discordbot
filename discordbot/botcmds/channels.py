...
PERM_VOICE_GUEST = discord.PermissionOverwrite(
    connect=True, speak=True, view_channel=True
)
...
PERM_TEXT_GUEST = discord.PermissionOverwrite(
    read_messages=True, send_messages=True, view_channel=True
)

CHANNEL_NAMES_RANDOM = [
    "RANDOM CHANNEL",
    "[RANDOM CHANNEL]",
    "[HP] SORTING HAT",
    "SORTING HAT",
...
async def get_category(
    dc_guild: discord.Guild, dj: DjangoConnection, raise_error=False
):
...
class ChannelsCog(commands.Cog, name="Channels"):
    ...
    async def on_voice_state_update(
        ...
        if (
            ...
            and before.channel.category.id == category_id
            and before.channel.id not in [private_id, public_id]
        ):
            try:
                db_member = await DB_Member.objects.filter(
                    server_id=member.guild.id,
                    **{
                        f"settings__{SETTING_MEMBER_VOI_ID}": before.channel.id
                    },
                ).aget()
        ...
        if (
            ...
            and after.channel.category.id == category_id
            and after.channel.id in [private_id, public_id]
        ):
        ...
        if (
            ...
            and after.channel.category
            and (
                "[RND]" in after.channel.name.upper()
                or "[RANDOM]" in after.channel.name.upper()
                or after.channel.name.upper() in CHANNEL_NAMES_RANDOM
            )
        ):
    ...
    async def text_channel_create(
        self, interaction: discord.Interaction, permission_type: Choice[str]
    ):
        ...
        if channel:
            raise ErrorMessage(
                "Du hast bereits einen Textkanal!",
                fields=[("Kanal", channel.mention)],
            )
        ...
        created_channel = await category.create_text_channel(
            ...
            overwrites={
                guild.default_role: (
                    PERM_TEXT_GUEST
                    if permission_type.value == "public"
                    else PERM_DEFAULT_NONE
                ),
        ...
        raise SuccessMessage(
            "Textkanal erstellt!",
            fields=[("Erstellter Kanal", created_channel.mention)],
        )
    ...
    async def text_channel_permissions(
        self,
        ...
        if target.id == self.bot.user.id:
            raise ErrorMessage(
                "Stopp! Lass mich selbst für meine Berechtigungen schauen!"
            )
        ...
        if action.value == "add":
            await channel.set_permissions(
                target,
                overwrite=PERM_TEXT_GUEST,
                reason="Benutzer hat Benutzer/Rolle eingeladen",
            )

            raise SuccessMessage(
                "Zu Textkanal eingeladen",
                fields=[
                    (
                        (
                            "Rolle"
            ...
            await channel.set_permissions(
                target,
                overwrite=PERM_DEFAULT_NONE,
                reason="Benutzer hat Benutzer/Rolle entfernt",
            )

            raise SuccessMessage(
                "Einladung zu Textkanal entzogen",
                fields=[
                    (
                        (
                            "Rolle"
    ...
    async def voice_channel_create(
        self, interaction: discord.Interaction, permission_type: Choice[str]
    ):
        await interaction.response.defer(ephemeral=True)
        ...
        if channel:
            ...
            raise ErrorMessage(
                "Du hast bereits einen Sprachkanal!",
                fields=[("Kanal", channel.mention)],
            )
    ...
    async def voice_channel_permissions(
        self,
        interaction: discord.Interaction,
        action: Choice[str],
        target: typing.Union[discord.Member, discord.Role],
        ...
        if action.value == "add":
            await channel.set_permissions(
                target,
                overwrite=PERM_VOICE_GUEST,
                reason="Benutzer hat Benutzer/Rolle eingeladen",
            )

            raise SuccessMessage(
                "Zu Sprachkanal eingeladen",
                fields=[
                    (
                        (
                            "Rolle"
            ...
            raise SuccessMessage(
                "Einladung zu Sprachkanal entzogen",
                fields=[
                    (
                        (
                            "Rolle"
    ...
    @app_commands.choices(
        action=[
            ...
            Choice(
                name="deactivate (alle Kanäle löschen)", value="deactivate"
            ),
        ]
    )
    async def channels_config(
        self, interaction: discord.Interaction, action: Choice[str]
    ):
        ...
        if action.value == "activate":
            ...
            channel_text = await category.create_text_channel(
                ...
                overwrites={
                    interaction.guild.default_role: PERM_CHANNEL_COMMANDS
                },
                reason=CHANNELS_ACTIVATE_REASON,
            ...
            channel_public = await category.create_voice_channel(
                ...
                overwrites={
                    interaction.guild.default_role: PERM_CHANNEL_AUTO_VOICE
                },
                reason=CHANNELS_ACTIVATE_REASON,
            )
            ...
            raise SuccessMessage(
                ...
                "Die Kategorie und Kanäle dürfen nach Belieben umbenannt werden.",
                fields=[
                    ("Erstellte Kategorie", category.mention),
                    ("Erstellter Textkanal für Befehle", channel_text.mention),
                    (
                        "Erstellter Sprachkanal für automatische öffentliche Kanäle",
                        channel_public.mention,
                    ),
                    (
                        "Erstellter Sprachkanal für automatische private Kanäle",
                        channel_private.mention,
                    ),
                ],
                inline=False,
            )
            ...
            for setting_name in [
                SETTING_SERVER_PRI_ID,
                SETTING_SERVER_PUB_ID,
                SETTING_SERVER_TXT_ID,
                SETTING_SERVER_CAT_ID,
            ]:
                category = interaction.guild.get_channel(
                    db_server.getSetting(setting_name)
                )
                if category:
