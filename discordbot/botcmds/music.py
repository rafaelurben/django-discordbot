...
class Music(commands.Cog):
            ...
            if before.channel and before.self_stream and not after.self_stream:
                ...
                if os.path.isfile(filepath):
                    ...
                    voice_client.play(
                        player,
                        after=lambda e: (
                            print("[Music] - Error: %s" % e) if e else None
                        ),
                    )
    ...
    async def on_message(self, message):
        if (
            MUSIC_MODULE
            and os.path.exists(MEMESPATH)
            and os.path.isdir(MEMESPATH)
        ):
            # *Help I need somebody Help*
            if (
                "i need somebody" in message.content.lower()
                and message.guild
        ...
        if os.path.exists(MEMESPATH) and os.path.isdir(MEMESPATH):
            ...
            async def meme(self, ctx, search: str = "windows-xp-error", *args):
                ...
                if result[1] >= 75:
                    ...
                    await ctx.sendEmbed(
                        title="Memetime!",
                        fields=[("Meme", str(filename).split(".")[0])],
                    )
                else:
                    raise ErrorMessage(
        ...
        async def play(self, ctx, search: str, *args):
            ...
            async with ctx.typing():
                ...
                if ctx.voice_client.is_playing():
                    ...
                    await player.send(
                        ctx, status="Song zur Playlist hinzugefügt!"
                    )
                else:
                    player.play(ctx)
