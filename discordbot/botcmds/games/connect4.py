...
from discordbot.models import VIERGEWINNT_NUMBER_EMOJIS, VierGewinntGame
...
class Connect4View(discord.ui.View):
    ...
    async def btn_callback(
        self, interaction: discord.Interaction, col: int = None
    ):
        ...
        if self.game.process(col, interaction.user.id):
            ...
            await interaction.response.send_message(
                YES + " Spielzug erfolgreich!", ephemeral=True, delete_after=3
            )
        else:
            await interaction.response.send_message(
                NO + " Spielzug fehlgeschlagen!",
                ephemeral=True,
                delete_after=3,
            )
...
class Connect4Cog(commands.Cog, name="Vier gewinnt"):
    ...
    group = app_commands.Group(
        name="connect4", description="Vier gewinnt in Discord"
    )
    ...
    async def cmd_duel(
        ...
        opponent: Member,
        width: app_commands.Range[int, 4, 10] = 7,
        height: app_commands.Range[int, 4, 10] = 6,
    ):
    ...
    async def cmd_games(
        ...
        challenges = VierGewinntGame.objects.filter(
            player_1_id=str(user.id), player_2_id__isnull=True
        ).order_by("-time_created")
        duels_created = VierGewinntGame.objects.filter(
            player_1_id=str(user.id), player_2_id__isnull=False
        ).order_by("-time_created")
        duels_invited = VierGewinntGame.objects.filter(
            player_2_id=str(user.id)
        ).order_by("-time_created")

        text_challenge = (
            ""
            if await challenges.aexists()
            else "Noch keine Challenges erstellt."
        )
            ...
            text_challenge += (
                (
                    (
                        ...
                        else YES if g.winner_id == str(user.id) else NO
                    )
                    if g.finished
                    else RUNNING
                )
                + f" ({g.id}) BOT "
                + g.time_created.strftime("%Y/%m/%d - %H:%M %Z")
                + "\n"
            )
        text_duels_created = (
            ""
            if await duels_created.aexists()
            else "Noch keine Duelle erstellt."
        )
        async for g in duels_created:
            text_duels_created += (
                ...
                + f" ({g.id}) <@{g.player_2_id}> "
                + g.time_created.strftime("%Y/%m/%d - %H:%M %Z")
                + "\n"
            )
        text_duels_invited = (
            ""
            if await duels_invited.aexists()
            else "Noch zu keinem Duell eingeladen."
        )
    ...
    async def cmd_reset(
        ...
        if (
            ...
            raise ErrorMessage(
                "Nur der Bot-Besitzer darf Spiele von anderen Spielern löschen!"
            )

        user = user or interaction.user
        await VierGewinntGame.objects.filter(
    ...
    async def cmd_resume(self, interaction: discord.Interaction, game_id: int):
        ...
        if not await VierGewinntGame.objects.filter(id=game_id).aexists():
            raise ErrorMessage(
                "Ein Spiel mit dieser ID konnte nicht gefunden werden! Möglicherweise gelöscht?"
            )

        game = await VierGewinntGame.objects.aget(id=game_id)

        if not game.finished and str(interaction.user.id) in [
            game.player_1_id,
            game.player_2_id,
        ]:
            ...
            try:
                ...
                description = (
                    f"Dieses Spiel wurde in einer [neuen Nachricht]({msg_ref.jump_url}) weitergeführt!\n\n"
    ...
    async def cmd_resume__game_id_autocomplete(
        self,
        interaction: discord.Interaction,
        current: str,
        ...
        async for game in VierGewinntGame.objects.filter(
            (
                Q(player_1_id=str(interaction.user.id))
                | Q(player_2_id=str(interaction.user.id))
            ),
            id__contains=f"{current}",
        ).order_by("finished")[:25]:
            name = f"(#{game.id}) Gestartet " + game.time_created.strftime(
                "%Y/%m/%d - %H:%M %Z"
            )
