...
_ytdl = youtube_dl.YoutubeDL(
    {
        ...
        "outtmpl": os.path.join(
            FILESPATH, "youtube", "%(extractor)s-%(id)s-%(title)s.%(ext)s"
        ),
...
class AudioManager:
    ...
    async def refetchUrlIfYouTube(self, watchurl: str):
        if "youtube.com" in watchurl:
            data = await self.loop.run_in_executor(
                None, lambda: _ytdl.extract_info(watchurl, download=False)
            )
            return [
                f
                for f in (
    ...
    async def getSongs(self, query: str):
        ...
        if data:
            if "entries" in data:
                return [
                    await self.database.getOrCreateAudioSourceFromDict(d)
        ...
        if data:
            ...
            if "entries" in data:
                if len(data["entries"]) > 1:
                    entries = list(
                        entry for entry in data["entries"] if not entry is None
                    )
                    return {
                        "title": "Playlist",
...
class YouTubePlayer(PCMVolumeTransformer):
    def __init__(self, filename, *, queue, data, stream=False, volume=0.5):
        source = FFmpegPCMAudio(
            filename, **(FFMPEG_OPTIONS_STREAM if stream else FFMPEG_OPTIONS)
        )
        ...
        await ctx.sendEmbed(
            ...
            description=(
                (
                    ...
                    else self.description[0:100] + "..."
                )
                if isinstance(self.description, str)
                else "Keine Beschreibung gefunden"
            ),
            color=ctx.cog.color,
            fields=fields,
    ...
    async def from_url(self, url, *, queue, loop=None, stream=False):
        ...
        data = await loop.run_in_executor(
            None, lambda: _ytdl.extract_info(url, download=not stream)
        )
